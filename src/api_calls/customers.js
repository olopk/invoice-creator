...
export const fetch_customers = () => {
    const graphqlQuery = {
        query: `{ getCustomers{ _id name nip city street phonenr selldate info hasInvoice}}`
    }
    return axios.post('/graphql', JSON.stringify(graphqlQuery))
    .then(response =>{
        const data = response.data.data.getCustomers.map((customer, index) => {
            return{ ...customer, selldate: customer.selldate.slice(0,10), key: index}
        })
        return {data: data}})
    .catch(err => {
        return {error: err}})
    ...
    const graphqlQuery = {
        query: ` 
            query FetchSingleCustomer($id: String!){
                getCustomer(id: $id){ _id name nip city street phonenr selldate info}}
        `,
        variables: { id: id }
    }
    return axios.post('/graphql', JSON.stringify(graphqlQuery))
